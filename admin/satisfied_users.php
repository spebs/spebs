<?php
$pagetitle = "Βαθμμός ικανοποίησης χρηστών";
require("header.php");

	echo "  <h3>Satisfied users for all ISPs</h3>";
	...
	$q = "SELECT round(round(1000*avgdown/purchased_bandwidth_dl_kbps,2)*100,0) ratio, COUNT(*) cons 
              ...
              WHERE workingDay=-1 AND peakHour=-1
              GROUP BY ratio";
	...
	echo "<hr/>";
        ...
        {
           echo "  <h3>Satisfied users for {$isp['name']}</h3>";
           $vaxis = '# of connections';
           $column_types = array('number','number');
           $q = "SELECT round(round(1000*avgdown/purchased_bandwidth_dl_kbps,2)*100,0) ratio, COUNT(*) cons
              FROM aggregation_per_connection m JOIN connection c ON m.connection_id=c.connection_id
              WHERE m.isp_id={$isp['isp_id']} AND workingDay=-1 AND peakHour=-1
              GROUP BY ratio";
